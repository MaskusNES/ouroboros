...
# VPS version: no Google Colab dependencies; reads secrets from env / .env file.
...
# ----------------------------
# 2) Data directories (local, no Drive mount)
# ----------------------------
DRIVE_ROOT = pathlib.Path(os.environ.get("DRIVE_ROOT", "/var/ouroboros/data")).resolve()
REPO_DIR = pathlib.Path(os.environ.get("REPO_DIR", "/opt/ouroboros_repo")).resolve()
                ...
                if mime_type.startswith("image/"):
                    ...
                    if raw_bytes:
                        ...
                        if file_text:
                            ...
                            # Save to local data dir and notify
                            drive_path = DRIVE_ROOT / "uploads" / fname
                            drive_path.parent.mkdir(parents=True, exist_ok=True)
                            with open(drive_path, "wb") as _f:
                                _f.write(raw_bytes)
                            text = f"[File saved to: {drive_path}] (format not auto-parseable: {detected_mime})" + (f"\nCaption: {caption}" if caption else "")
                    else:
